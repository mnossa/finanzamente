...
class DashboardBoardController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($count >= $limit) {
            return back()->withErrors([
                'name' => "Hai raggiunto il limite di {$limit} dashboard per il tuo piano.",
            ]);
        }
    ...
    {
        if ($user->isPro()) {
            return (int) config('plans.pro_limits.max_dashboard_boards', 10);
        }

        return (int) config('plans.base_limits.max_dashboard_boards', 2);
    }
}
