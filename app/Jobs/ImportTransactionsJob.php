...
class ImportTransactionsJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        ] = DB::transaction(function () use ($converter, $importRecord, $defaultCurrency, $accountBalanceService): array {
            return UpdateAccountBalance::withoutBalanceSync(function () use ($converter, $importRecord, $defaultCurrency, $accountBalanceService): array {
                if ($importRecord) {
                    $importRecord->update([
                        'status' => 'processing',
                        'started_at' => now(),
                ...
                // Cache conti toccati (saldo ricalcolato una volta a fine import)
                $accountsCache = [];

                $loadAccount = function (int $id) use (&$accountsCache): ?Account {
                ...
                // Un solo ricalcolo SUM per conto toccato (niente N listener sync)
                foreach ($accountsCache as $account) {
                    $accountBalanceService->syncStoredBalance($account);
                }
