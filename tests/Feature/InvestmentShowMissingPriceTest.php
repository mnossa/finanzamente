...
class InvestmentShowMissingPriceTest extends TestCase
{
    ...
    public function show_returns_ok_when_market_price_unavailable_for_btp(): void
    {
        ...
        $this->mock(AssetPriceService::class, function ($mock) {
            $mock->shouldReceive('getCurrentPrice')
                ->once()
                ->with('IT0005672024')
                ->andReturn(['error' => 'Simbolo non trovato', 'price' => null]);
        });
    ...
    #[Test]
    public function show_survives_price_provider_throwable(): void
    {
        $user = User::factory()->create([
            'email_verified_at' => now(),
            'plan' => 'pro',
        ...
        $this->mock(AssetPriceService::class, function ($mock) {
            $mock->shouldReceive('getCurrentPrice')
                ->once()
                ->andThrow(new \TypeError('Cannot convert array to float'));
        });
