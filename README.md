...
make test-ci     # Esegue i gate CI locali: Pint (--test) + suite test
make pint-fix    # Corregge automaticamente lo stile PHP con Pint
```
...
make test-ci                # Replica i controlli CI (Pint + test)
make pint-check             # Verifica stile PHP (non modifica file)
make pint-fix               # Applica formattazione automatica Pint
...
Per evitare differenze tra locale e GitHub Actions, eseguire `make test-ci` prima del push.
